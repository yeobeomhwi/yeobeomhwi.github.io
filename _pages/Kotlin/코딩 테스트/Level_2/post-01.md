...
### 입력
    첫번째 줄에 주어지는 정수의수 N과 플레이어가 찾으려는 정수의 위치 K가 공백을 두고 주어진다. <br>둘째중에 정수 A1,A2....An이 공백을 두고 주어진다
...
예를 들어, 다음과 같이 사용합니다:
``` kotlin
val sortedList = list.sortedWith(compareByDescending { it.property1 }.thenByDescending { it.property2 })
```
여기서:
